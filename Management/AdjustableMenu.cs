using BepInEx; // Not strictly needed here unless using BepInEx-specific features directly
using System;
using System.Collections.Generic;
using System.Linq;
using TextUITemplate.Libraries; // For Interfaces, ControllerInputs
using TextUITemplate.Mods;    // Required to access WeatherMod.TimeOptions
using TMPro;
using UnityEngine;
...
namespace TextUITemplate.Management
{
    public class AdjustableMenu
    {
        ...
        private static int selectedUiItemIndex = 0; // Index for items within this adjustable menu (values + presets row)
        private static float inputCooldown = 0f;
        private const float COOLDOWN_TIME = 0.15f; // Cooldown for joystick inputs

        private static Button currentFocusedModButton = null; // The button from the main menu that has focus
        ...
        private static int currentPresetCycleIndex = 0; // For cycling through presets of the focused mod

        private const string PRESET_ITEM_NAME = "Apply Preset"; // Display name for the preset selection row
        ...
        public static void SetFocusedMod(Button modButton)
        {
            ...
            if (modChanged)
            {
                selectedUiItemIndex = 0; // Reset selection within adjustable menu
                currentPresetCycleIndex = 0; // Reset preset cycle
            }
        ...
        {
            if (!Menu.IsMenuToggledOn) // Main menu must be on
            {
            ...
            if (Time.time >= inputCooldown && totalUiItems > 0) //
            {
                ...
                else if (selectedUiItemIndex < currentModValues.Count) // Handle AdjustableValue interaction
                {
                    ...
                    if (ControllerInputs.LeftStickRight() || UnityInput.Current.GetKey(KeyCode.KeypadPlus) || UnityInput.Current.GetKey(KeyCode.Equals)) //
                    {
                        switch (selectedVal.ValType)
                        {
                            ...
                            case AdjustableValueType.Int:
                                int currentIntVal = (int)originalValue;
                                int nextIntVal = currentIntVal + selectedVal.IntIncrementStep;
                                if (selectedVal.Name == "Time of Day") // Special handling for Time of Day to cycle
                                {
                                    newValue = (nextIntVal > selectedVal.MaxValue) ? (int)selectedVal.MinValue : nextIntVal;
                                }
                                else
                                {
                                    newValue = Mathf.Clamp(nextIntVal, (int)selectedVal.MinValue, (int)selectedVal.MaxValue);
                                }
                                break;
                    ...
                    else if (ControllerInputs.LeftStickLeft() || UnityInput.Current.GetKey(KeyCode.KeypadMinus) || UnityInput.Current.GetKey(KeyCode.Minus)) //
                    {
                        switch (selectedVal.ValType)
                        {
                            ...
                            case AdjustableValueType.Int:
                                int currentIntVal = (int)originalValue;
                                int prevIntVal = currentIntVal - selectedVal.IntIncrementStep;
                                if (selectedVal.Name == "Time of Day") // Special handling for Time of Day to cycle
                                {
                                    newValue = (prevIntVal < selectedVal.MinValue) ? (int)selectedVal.MaxValue : prevIntVal;
                                }
                                else
                                {
                                    newValue = Mathf.Clamp(prevIntVal, (int)selectedVal.MinValue, (int)selectedVal.MaxValue);
                                }
                                break;
                            case AdjustableValueType.Bool: newValue = !(bool)originalValue; break; //
                        }
            ...
            else
            {
                for (int i = 0; i < currentModValues.Count; i++) //
                {
                    ...
                    if (valToDisplay.Name == "Time of Day" && valToDisplay.ValType == AdjustableValueType.Int)
                    {
                        int timeIndex = (int)currentValueObj;
                        if (timeIndex >= 0 && timeIndex < WeatherMod.TimeOptions.Length)
                            valueString = WeatherMod.TimeOptions[timeIndex];
                        else
                            valueString = "Invalid Time"; // Should not happen with MinValue/MaxValue
                    }
                    else // Existing logic for other types
                    {
            ...
            foreach (var presetValueEntry in presetToApply.Values) //
            {
                ...
                if (targetAdjustableValue != null)
                {
                    try
                    {
                        ...
                        // Basic type conversion, can be expanded if needed
